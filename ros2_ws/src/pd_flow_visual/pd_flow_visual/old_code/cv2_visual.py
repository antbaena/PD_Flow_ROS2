...
class CV2Visual(Node):
    ...
    def display_flow_field(self, image_height, image_width, rgb_image, depth_image, dx, dy, dz, color_map):
        ...
        u, v = np.arange(image_height), np.arange(image_width)
        U, V = np.meshgrid(u, v)

        # Obtener los valores de profundidad y hacer la escala
        Z = depth_image[V, U]

        # Aplanar las matrices para que coincidan con las dimensiones de dx, dy, dz
        X = (U - u0) * Z / f
        Y = (V - v0) * Z / f

        # Convertir las matrices a vectores unidimensionales
        X_flat = X.flatten()
        Y_flat = Y.flatten()
        Z_flat = Z.flatten()

        # Ajustar el tamaño de dx, dy, dz
        dx_flat = dx
        dy_flat = dy
        dz_flat = dz

        # Calcular los puntos 3D actualizados
        X_new_flat = X_flat + dx_flat
        Y_new_flat = Y_flat + dy_flat
        Z_new_flat = Z_flat + dz_flat

        # Rehacer las matrices de las nuevas coordenadas
        X_new = X_new_flat.reshape((image_width, image_height))
        Y_new = Y_new_flat.reshape((image_width, image_height))
        Z_new = Z_new_flat.reshape((image_width, image_height))

        # Proyección del punto 3D actualizado a la imagen 2D
        valid_indices = Z_new != 0
        new_x = np.round((X_new * f / Z_new) + u0).astype(int)
        new_y = np.round((Y_new * f / Z_new) + v0).astype(int)

        # Asegurarse de que los puntos estén dentro de los límites de la imagen
        valid_indices &= (new_x >= 0) & (new_x < image_width) & (new_y >= 0) & (new_y < image_height)

        # Obtener los colores de los píxeles originales
        colors = rgb_image[V[valid_indices], U[valid_indices]]
###################################### Va todo bien hasta que llega aqui, que pone que IndexError: index 480 is out of bounds for axis 0 with size 480 

        print(colors.shape, image.shape, new_x.shape, new_y.shape)
        # Crear una imagen para la visualización
        image[new_y[valid_indices], new_x[valid_indices]] = colors
