...
int main(int num_arg, char *argv[])
{
    ...
    // Define el cronómetro
    steady_clock::time_point start, end;
    float elapsed_time;

    while (!stop)
    {
        char pushed_key = sceneflow.getKeyPressed();

        switch (pushed_key) {
        // Capture new frame
        case 'n':
            sceneflow.CaptureFrame();
            start = steady_clock::now();  // Inicia el cronómetro
            sceneflow.createImagePyramidGPU();
            sceneflow.solveSceneFlowGPU();
            end = steady_clock::now();  // Detiene el cronómetro
            elapsed_time = duration<float, milli>(end - start).count(); // Calcula el tiempo en ms
            cout << endl << "PD-Flow runtime: " << elapsed_time;
        ...
        case 's':
            working = !working;
            start = steady_clock::now();  // Reinicia el cronómetro
            break;
        ...
        if (working)
        {
            while (true) {
                ...
                elapsed_time = duration<float>(end - start).count();  // Calcula el tiempo en segundos
                if (elapsed_time >= 1.0f / sceneflow.fps) break;
                this_thread::sleep_for(milliseconds(1));  // Evita el uso intensivo de CPU
            }

            const float exec_time = duration<float>(end - start).count();
            start = steady_clock::now();  // Reinicia el cronómetro
            ...
            const float total_time = duration<float, milli>(end - start).count();  // Tiempo en ms
